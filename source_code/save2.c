...
my_thread (void *thread_parameter)
{
	...
	float old, new;
    int width = grid_2->dim-2;
    int num_row = width/num_threads;
    int row_range = 0;
    //int duty = 0; //number of element for each thread. Last thread have extra element.
    if ((parameter->thread_idx) != num_threads-1){ 
        //duty = width * num_row;
        row_range = num_row;
    }
    if ((parameter->thread_idx) == num_threads -1)
    {
        row_range = width - (num_threads-1)*num_row;
        //duty = row_range*width;
    }
    ...
    while (!done2){
        ...
        int j = 1;
        int i = (parameter->thread_idx) * num_row + 1; //starting index of matrix for each thread
        //printf ("Start i %d, j %d\n", i, j);
        for (i = (parameter->thread_idx) * num_row + 1; i < (parameter->thread_idx) * num_row + 1 + row_range; i++) {
            for ( j = 1; j < (grid_2->dim - 1); j++) {
        ...
        //printf ("End i %d, j %d element_count %d\n", i, j, element_count);
        num_elements2+=element_count;
        diff2 += diff_temp;
        printf ("diff %f at barrier of Thread %d\n", diff2, parameter->thread_idx);
